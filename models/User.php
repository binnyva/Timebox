...
class User extends DBTable {
	...
	function User() {
		...
		parent::__construct($this->user_table);
		...
		if(isset($_COOKIE['username']) and $_COOKIE['username'] and isset($_COOKIE['password_hash'])) {
			$user_details = $sql->getAssoc("SELECT id,name FROM {$this->user_table} 
					WHERE username='$_COOKIE[username]' AND MD5(CONCAT(password, '{this->password_salt}'))='$_COOKIE[password_hash]'");
	...
	function login($username,$password,$remember=0) {
		...
		$user_details = $sql->getAssoc("SELECT id,name FROM {$this->user_table} WHERE username='$username' AND password='$password'");
		if(!$user_details) { //Query did not run correctly
			...
			if($remember) {
				...
				setcookie('password_hash', md5($password.$this->password_salt), $this->cookie_expire,'/');
			}
		}
	...
	function register($username, $password, $name, $email, $url) {
		...
		$result 	= $sql->getSql("SELECT id FROM {$this->user_table} WHERE username='$username'");
	 ...
	 */
	 function passwordRetrival($data) {
	 	...
	 	if(isset($data['username'])) {
		 	extract($sql->getAssoc("SELECT name,username,password,email FROM {$this->user_table} WHERE username='$data[username]'"));
	 	} elseif(isset($data['email'])) {
	 		extract($sql->getAssoc("SELECT name,username,password,email FROM {$this->user_table} WHERE email='$data[email]'"));
	 	} else {
